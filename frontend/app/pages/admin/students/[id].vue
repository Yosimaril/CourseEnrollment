<template>
    <main class="min-h-screen bg-slate-50 px-4 py-6">
        <section class="mx-auto w-full max-w-2xl rounded-md bg-white p-6 shadow-md sm:p-8">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <p class="text-sm font-bold uppercase tracking-[0.08em] text-slate-600">Admin students</p>
                    <h1 class="mt-2 text-3xl font-bold text-slate-900">Update student</h1>
                    <p class="mt-2 max-w-2xl text-base leading-7 text-slate-600">Edit the student account details and save the changes.</p>
                </div>

                <NuxtLink to="/admin/students" class="inline-flex min-h-11 items-center justify-center rounded-md border border-slate-200 bg-white px-4 py-2 font-semibold text-slate-900 transition hover:border-blue-900"> Back to list </NuxtLink>
            </div>

            <p v-if="isLoading" class="mt-8 text-slate-600">Loading student...</p>
            <p v-else-if="errorMessage" class="mt-8 rounded-md border border-red-200 bg-red-50 px-4 py-3 text-red-700">
                {{ errorMessage }}
            </p>

            <form v-else class="mt-8 space-y-4" @submit.prevent="submitUpdate">
                <label class="block space-y-2">
                    <span class="block text-md font-semibold text-slate-900">Username</span>
                    <input v-model.trim="form.username" type="text" class="w-full rounded-md border border-slate-200 bg-slate-100 px-4 py-2 outline-none transition focus:border-blue-900" required />
                </label>

                <label class="block space-y-2">
                    <span class="block text-md font-semibold text-slate-900">Email</span>
                    <input v-model.trim="form.email" type="email" class="w-full rounded-md border border-slate-200 bg-slate-100 px-4 py-2 outline-none transition focus:border-blue-900" required />
                </label>

                <label class="block space-y-2">
                    <span class="block text-md font-semibold text-slate-900">Role</span>
                    <select v-model="form.role" class="w-full rounded-md border border-slate-200 bg-slate-100 px-4 py-2 outline-none transition focus:border-blue-900" required>
                        <option :value="UserRoleEnum.STUDENT">Student</option>
                        <option :value="UserRoleEnum.ADMIN">Admin</option>
                    </select>
                </label>

                <label class="block space-y-2">
                    <span class="block text-md font-semibold text-slate-900">NRP</span>
                    <input v-model.trim="form.nrp" type="text" class="w-full rounded-md border border-slate-200 bg-slate-100 px-4 py-2 outline-none transition focus:border-blue-900" />
                </label>

                <label class="block space-y-2">
                    <span class="block text-md font-semibold text-slate-900">Max credits</span>
                    <input v-model="form.maxCredits" type="number" min="0" step="1" class="w-full rounded-md border border-slate-200 bg-slate-100 px-4 py-2 outline-none transition focus:border-blue-900" />
                </label>

                <p v-if="submitErrorMessage" class="mt-1 text-md text-red-700">{{ submitErrorMessage }}</p>

                <button type="submit" class="mt-2 w-full rounded-md bg-blue-900 px-4 py-2 font-semibold text-white transition hover:bg-blue-800 disabled:cursor-not-allowed disabled:opacity-75" :disabled="isSubmitting">
                    {{ isSubmitting ? "Updating..." : "Update student" }}
                </button>
            </form>
        </section>
    </main>
</template>

<script setup lang="ts">
import { UserRoleEnum } from "~/constants/userRoleEnum";
import type { Student } from "~/models/student";
import { StudentService } from "~/services/student";
import { extractErrorMessage } from "~/utils/auth";

definePageMeta({
    middleware: ["auth", "admin"],
});

const route = useRoute();
const studentId = Number(route.params.id);

const form = reactive({
    username: "",
    email: "",
    role: UserRoleEnum.STUDENT,
    nrp: "",
    maxCredits: "",
});

const isLoading = ref(true);
const isSubmitting = ref(false);
const errorMessage = ref("");
const submitErrorMessage = ref("");

onMounted(async () => {
    try {
        const response: Student = await StudentService.getStudent(studentId);
        form.username = response.username;
        form.email = response.email;
        form.role = response.role;
        form.nrp = response.nrp ?? "";
        form.maxCredits = response.maxCredits !== undefined && response.maxCredits !== null ? String(response.maxCredits) : "";
    } catch (error) {
        errorMessage.value = extractErrorMessage(error, "Unable to load student right now.");
    } finally {
        isLoading.value = false;
    }
});

async function submitUpdate() {
    isSubmitting.value = true;
    submitErrorMessage.value = "";

    try {
        await StudentService.updateStudent(studentId, {
            username: form.username,
            email: form.email,
            role: form.role,
            nrp: form.nrp ? form.nrp : undefined,
            max_credits: form.maxCredits !== "" ? Number(form.maxCredits) : undefined,
        });

        await navigateTo({ path: "/admin/students", query: { refresh: Date.now().toString() } });
    } catch (error) {
        submitErrorMessage.value = extractErrorMessage(error, "Unable to update student right now.");
    } finally {
        isSubmitting.value = false;
    }
}
</script>
