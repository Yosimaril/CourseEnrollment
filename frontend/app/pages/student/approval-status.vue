        ...
        <section class="mx-auto w-full max-w-6xl rounded-md bg-white p-6 shadow-md sm:p-8">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    ...
                    <p class="mt-2 max-w-2xl text-base leading-7 text-slate-600">Review every course plan and cancel any request that has not been approved yet.</p>
                </div>

                <NuxtLink to="/student/dashboard" class="inline-flex min-h-11 items-center justify-center rounded-md border border-slate-200 bg-white px-4 py-2 font-semibold text-slate-900 transition hover:border-blue-900"> Back to dashboard </NuxtLink>
            </div>
            ...
            <div v-else class="mt-8 space-y-4">
                <div v-if="plans.length === 0" class="rounded-md border border-slate-200 bg-slate-50 p-5 text-slate-600">No course plan history yet.</div>
                    ...
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            ...
                            <h2 class="mt-2 text-2xl font-bold text-slate-900">
                                {{ CoursePlanStatusMapper[plan.status] }}
                            </h2>
                            <div class="mt-3 space-y-1">
                                <p class="text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">Last updated</p>

                                <p class="text-sm text-slate-700">
                                    {{ formatDate(plan.updatedAt) }}
                                </p>
                            </div>
                            <p class="mt-2 text-sm text-slate-600">{{ plan.items.length }} item{{ plan.items.length === 1 ? "" : "s" }}</p>
                        </div>

                        <button v-if="canCancel(plan.status)" type="button" class="inline-flex min-h-11 items-center justify-center rounded-md border border-slate-200 bg-white px-4 py-2 font-semibold text-slate-900 transition hover:border-blue-900 disabled:cursor-not-allowed disabled:opacity-75" :disabled="pendingCancelIds.includes(plan.id)" @click="cancelPlan(plan.id)">
                    ...
                    <div class="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                        <div v-for="item in plan.items" :key="`${plan.id}-${item.courseId}`" class="rounded-md border border-slate-200 bg-white p-4">
                            ...
                            <span
                                ...
                                :class="{
                                    'bg-yellow-100 text-yellow-800 border-yellow-200': CoursePlanItemStatusMapper[item.status].color === 'yellow',
                                    'bg-green-100 text-green-800 border-green-200': CoursePlanItemStatusMapper[item.status].color === 'green',
                                    'bg-red-100 text-red-800 border-red-200': CoursePlanItemStatusMapper[item.status].color === 'red',
                                }"
                            >
                                {{ CoursePlanItemStatusMapper[item.status].label }}
                            </span>
                        </div>
                    </div>
                </article>
